  ...
  fn =
    {
      ...
      wrapRc ? true,
      src ? self.lib.nvimPath, # Use this repo as src?
    ...
    let
      ...
      _mkNeovim =
        let
          ...
          indent =
            ...
            in
            lib.concatStringsSep "\n" (map (line: if line == "" then line else prefix + line) lines);
          ...
          setupLuaRc =
            ...
            let
              ...
              chosen-init = if dynamicConfig then dynCfgInit else init;
        ...
        in
        (wrapped.overrideAttrs overrideAttrs).overrideAttrs (prev: {
          passthru = lib.recursiveUpdate prev.passthru rc;
        });
      ...
      normalizePlugins =
        defaults:
        let
          coerceToPlugin = x: defaults // (if x ? plugin then x else { plugin = x; });
        in
        (map coerceToPlugin);
      ...
      when = cond: x: if cond then x else null;
      ...
      appName' = if (appName == null || appName == "") then "nvim" else appName;
