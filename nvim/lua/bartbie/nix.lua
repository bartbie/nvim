...
local function set_plugins_loader(base_path)
    table.insert(package.loaders, 2, function(module_name)
        ...
        if vim.fs.dirname(module_path) ~= "plugins" then
            return nil
        end

        local parent = vim.fs.joinpath(base_path, module_path)
        for _, ending in ipairs({ ".lua", "/init.lua" }) do
            local path = vim.fs.normalize(parent .. ending)
...
function M.set_plugins_loader()
    local rtp = assert(M.get_roots().rtp_root)
